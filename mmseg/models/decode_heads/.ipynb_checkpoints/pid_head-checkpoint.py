...
class BasePIDHead(BaseModule):
    ...
    Args:
        ...
        init_cfg (dict or list[dict], optional): Init config.
            Default: None.
    """
...
class PIDHead(BaseDecodeHead):
    """Decode head for PIDNet (Binary Segmentation Adapted)."""
    ...
    def loss_by_feat(self, seg_logits: Tuple[Tensor],
        ...
        # 尺寸对齐
        p_logit = resize(p_logit, size=sem_label.shape[2:], mode='bilinear', align_corners=self.align_corners)
        i_logit = resize(i_logit, size=sem_label.shape[2:], mode='bilinear', align_corners=self.align_corners)
        d_logit = resize(d_logit, size=bd_label.shape[2:], mode='bilinear', align_corners=self.align_corners)

        sem_label = sem_label.squeeze(1)
        bd_label = bd_label.squeeze(1)

        # ---------------------------
        # 🔥 二分类安全版本（只保留 3 个 loss）
        # ---------------------------
        loss['loss_sem_p'] = self.loss_decode[0](p_logit, sem_label, ignore_index=self.ignore_index)
        loss['loss_sem_i'] = self.loss_decode[1](i_logit, sem_label, ignore_index=self.ignore_index)
        loss['loss_bd'] = self.loss_decode[2](d_logit, bd_label)

        # ---------------------------
        # 🔥 移除第 4 个 loss，解决 index 3 报错
        # ---------------------------
        # loss['loss_sem_bd'] = self.loss_decode[3](...)  # 已删除
